...
引擎和 UI 向 GTK 后备窗发送的是独立快照，写端为非阻塞管道。后备窗失去响应或管道积压时，TiPE 会丢弃旧通道并在下一次更新重建窗口，不允许候选窗反向卡住 fcitx5 的按键线程。
...
模型适配器还会从有界监督历史中统计重复确认的纠错链。请求中的 `top_corrections` 保存高频“误拼音 -> 修正拼音”对；与当前输入有关的条目会转换成 `history_correction` 证据，再经过本地阈值和合法性校验，不能凭一条历史记录直接改写候选。

### 3. TiP 与外部模型

TiP 是 TiPE 自带的个人行为模型。训练时读取终止输入记录，生成带版本和验证结果的模型文件。通用排序权重必须通过隔离验证才能替换；当当前有界历史不足以训练出更好的完整排序器时，发布流程以旧模型为基底，只单调合并经过边界校验的精确选词、英文词和键盘纠错证据，各项证据达到独立阈值后才影响输入。运行时再把安全能力蒸馏成 C++ 可读取的有界规则，不需要每次按键启动 Python。
