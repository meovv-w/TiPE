...
TiP 训练读取有界监督历史，生成版本化 JSON 模型。完整排序器只有在隔离验证中达到发布门槛时才替换旧版本；即使新排序器暂时没有收益，经过边界校验的精确选词、英文词和按键纠错证据仍会安全合并到旧模型，达到各自阈值后才影响输入。合并使用单调最大计数，同一批有界记录重复训练不会重复累计。训练结束后，安全能力还会蒸馏成运行时偏好行，让 C++ 引擎直接使用；普通按键不会重复启动训练脚本。
...
- 不包含应用名称。
- 不包含原始周边文字。
- 包含受限的纠错轨迹和已经过本地校验的证据，用于安全增量学习。
- 读取 Fedora/Rime 拼音词典作为可用先验，但不上传词典内容。

点击“更新 TiP”只训练累计的终止样本，不分析当前正在输入的文字。“分析当前输入”是另一项独立操作。

如果窗口提示完整排序没有可验证提升，TiP 会继续使用更可靠的旧排序器。只有同时没有新增达到安全阈值的个人证据时，才会显示本次未更新；这不等于监督记录丢失或训练失败。
